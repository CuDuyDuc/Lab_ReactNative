...
const YourComponent: React.FC = ({ navigation }: any) => {
  ...
  const onGetUserList = async () => {
    try {
      const response = await axios.get('http://192.168.1.8:3000/user');
  ...
  const onDeleteUser = async (userId: number) => {
    try {
      await axios.delete(`http://192.168.1.8:3000/user/${userId}`);
      onGetUserList();
    } catch (error) {
  ...
  const onUpdateUser = async () => {
    try {
      ...
      const response = await axios.put(`http://192.168.1.8:3000/user/${selectedUserId}`, {
